...
//				[textBoxWidth = uiMeshComponent->GetWidthLocal(), visibleStartLineIndex = mVisibleLineStartIndex, visibleLineNum = mVisibleLineNum](TextCharacterPanelEntity* panel) {
//
//					//mVisibleLineNum;
...
//		{
//			std::for_each(mTextCharacterPanelEntityVector.begin(), mTextCharacterPanelEntityVector.begin() + mText.size(),
//				[textBoxWidth = uiMeshComponent->GetWidthLocal()](TextCharacterPanelEntity* panel) {
//
//					//mVisibleLineNum;
//					// mVisibleLineStartIndex;
//
...
//	}
//
//	GetUiMeshComponent()->SetWidthHeightLocal(width, height);
//
...
//	mTextColor = color;
//
//	std::for_each(mTextCharacterPanelEntityVector.begin(), mTextCharacterPanelEntityVector.begin()+mText.size(), [&color](TextCharacterPanelEntity* panelEntity) {panelEntity->GetCharacterEntity()->GetUiMeshComponent()->SetColor(color); });
//
//}
//
...
//
//
//void Quad::TextBox::HandleHover(const MouseInputData & mouseInputData)
//{
//
...
//}
//
//void Quad::TextBox::HandleLButtonDown(const MouseInputData& mouseInputData)
//{
//
//	//어디클릭했는가?
//
...
//	if(uiComponent->GetHoverState()  &&  GetTextEditMode() == false)
//	{
//		SetTextEditMode(true);
//	}
//	else if( !uiComponent->GetHoverState() && GetTextEditMode() == true)
//	{
//
//		SetTextEditMode(false);
//	}
//
//	if (uiComponent->GetHoverState() == false)
//		return;
//
...
//
//	//MouseEvent* mouseEvent = (MouseEvent*)pEvent;
//	SetCursorPosition(mouseInputData);
//
...
//}
//
//void Quad::TextBox::HandleMouseMove(const MouseInputData& mouseInputData)
//{
//	UiComponent* uiComponent = GetUiComponent();
//
//	//StateComponent* stateComponent = GetComponent<StateComponent>();
//	
//
//	if (uiComponent->GetLButtonDownState())
//	{
//
//		
//
...
//}
//
//void Quad::TextBox::HandleLButtonUp(const MouseInputData& mouseInputData)
//{
////	StateComponent* stateComponent = GetComponent<StateComponent>();
//	UiComponent* uiComponent = GetUiComponent();
//
...
//{
//	DirectX::XMFLOAT3 textBoxWorldPos = GetRootSceneComponent()->GetTranslationWorld();
//	//GetTransform().GetPositionWorld();
//
//	//newCharacterPanelEntity->SetSystem(GetSystem());
//
//
//	UiComponent * newCharacterPanelEntityUIComponent =	newCharacterPanelEntity->GetUiComponent();
//	UIMeshComponent * newCharacterPanelEntityUiMeshComponent = newCharacterPanelEntity->GetUiMeshComponent();
//
//	int cursorPositionInText = mTextCursorEntity->GetPositionInText();
//	
//	//newCharacterPanelEntity->Initialize(20, GetLineHeight());
//	//newCharacterPanelEntity->SetSelectAvailableFlag(false);
//	newCharacterPanelEntityUIComponent->SetSelectAvailableFlag(false);
//
//
//	//newCharacterPanelEntity->GetTransform().SetIndependentScaleFlag(true);
//	newCharacterPanelEntityUiMeshComponent->SetIndependentScaleFlag(true);
//
//	newCharacterPanelEntity->GetCharacterEntity()->SetDrawFlag(true);
//	newCharacterPanelEntity->GetCharacterEntity()->GetUiMeshComponent()->SetColor(GetUiMeshComponent()->GetColor());
...
//			//이전글자가 엔터인가?
//			//textbox경계를 넘어가는가?
//		if (textLineMode == ETextLineMode::eMultiLine  && (beforeCharacterPanelLocalPosition.x + currentCharacterAdvance > (uiMeshComponent->GetWidthLocal() / 2) || preCharacter == L'\r'))
//		{
//			//라인이 넘어갔음으로 
//			//밑줄시작으로
...
//		}
//
//		mTextCharacterPanelEntityVector[characterEntityIndex]->SetObjectPositionLocal(beforeCharacterPanelLocalPosition.x,
...
//	else
//	{
//		mTextCharacterPanelEntityVector[characterEntityIndex]->SetObjectPositionLocal(mCharacterPanelStartLocalPosX + currentCharacterAdvance / 2, mCharacterPanelStartLocalPosY - GetLineHeight() / 2, -1.0F);
//
...
//	{
//
//		UIMeshComponent * textCharacterPanelEntityMeshComponent=	(*lineFirstIt)->GetUiMeshComponent();
//
//
...
//		pos.x = -1.0f * panelWidthSum / 2 + textCharacterPanelEntityMeshComponent->GetWidthLocal() / 2;
...
//			pos.x += (*(lineIt - 1))->GetUiMeshComponent()->GetWidthLocal() / 2 + (*lineIt)->GetUiMeshComponent()->GetWidthLocal() / 2;
//
//			//현재문자위치설정
...
//		{
//			//그라인의 모든문자들의 너비합 
//			panelWidthSum += (*it)->GetUiMeshComponent()->GetWidthLocal();
//
//		}
//		
//
...
//
//	case ETextAlignmentDirection::eRight:
//		startPosX = mCharacterPanelStartLocalPosX + GetUiMeshComponent()->GetWidthLocal() / 2;
//		break;
//	}
//
//
...
//
//		//정렬에따라 다르다.
//		cursorPos.x = startPosX + mTextCursorEntity->GetUiMeshComponent()->GetWidthLocal()/2;
...
//			cursorPos.x += panel->GetUiMeshComponent()->GetWidthLocal() / 2;
//		}
//
//
//	}
...
//		cursorPos = panel->GetTransform().GetPositionLocal();
//		cursorPos.x -= panel->GetUiMeshComponent()->GetWidthLocal() / 2;
//		cursorPos.x += mTextCursorEntity->GetUiMeshComponent()->GetWidthLocal() / 2;//보정; //0일떄 경계안으로들어오록하기위해
//	}
//
//	mTextCursorEntity->GetTransform().SetPositionLocal(cursorPos);
//
...
//	{
//
//		float textBoxWidth = GetUiMeshComponent()->GetWidthLocal();
//
...
//		float cursorWidth = mTextCursorEntity->GetUiMeshComponent()->GetWidthLocal();
...
//			float lastCharacterWidth = mTextCharacterPanelEntityVector[mText.size() - 1]->GetUiMeshComponent()->GetWidthLocal();
//			float firstCharacterPosX = mTextCharacterPanelEntityVector[0]->GetTransform().GetPositionLocal().x;
//			float firstCharacterWidth = mTextCharacterPanelEntityVector[0]->GetUiMeshComponent()->GetWidthLocal();
//
//			//더 짧은거리만큼 이동한다.
//
...
//
//}
//void Quad::TextBox::SetCursorPosition(const MouseInputData& mouseInputData)
//{
//
//	if (mText.size() == 0)
//	{
...
//	Ray ray ;
//	Map* map = GetMap();
//	CameraHelper::CalculateRay({ (float)mouseInputData.mClientPositionX ,(float)mouseInputData.mClientPositionY }, map->GetViewPort(GetMapLayerID()),
//		*map->GetMainCamera(GetMapLayerID()), true, ray);
//
//
...
//	float visibleLineY = (*firstPanelEntityIt)->GetRootSceneComponent()->GetTranslationWorld().y;
...
//
//	if (((*panelEntityIt)->GetRootSceneComponent()->GetTranslationWorld().x - ((*panelEntityIt)->GetUiMeshComponent()->GetWidthLocal() / 2) > mouseWorldX))
//	{
...
//		{
//
//			UIMeshComponent* meshComponent = (*panelEntityIt)->GetUiMeshComponent();
//
//			if (CollisionHelper::Intersect(meshComponent->GetCollider(), ray, t))
//			{
//				//이 문자를 클릭했다.
//
//				//그문자의 중심에서 어느쪽(x축)에 치우쳐서 클릭되었는가를 판단해서 (왼쪽이냐 오른쪽이냐) , 커서위치 최종확정 
//				int panelIndex = panelEntityIt - mTextCharacterPanelEntityVector.begin();
//				float panelWorldX = (*panelEntityIt)->GetRootSceneComponent()->GetTranslationWorld().x;
...
//		while (1)
//		{
//			float characterPosX = (*panelEntityIt)->GetRootSceneComponent()->GetTranslationWorld().x;
//			float characterWidth = (*panelEntityIt)->GetUiMeshComponent()->GetWidthLocal();
//
...
//
//	std::for_each(mTextCharacterPanelEntityVector.begin(), mTextCharacterPanelEntityVector.begin() + mText.size(),
//		[textBoxWidth = GetUiMeshComponent()->GetWidthLocal()](TextCharacterPanelEntity* panel) {
//
//			wchar_t ch = panel->GetCharacter();
//			float chPosX = panel->GetTransform().GetPositionLocal().x;
//			float chWidth = panel->GetUiMeshComponent()->GetWidthLocal();
//			if ((ch != L' ') && ((chPosX + chWidth / 2) >= -textBoxWidth / 2) && ((chPosX - chWidth / 2) <= textBoxWidth / 2))
//			{
//				panel->GetCharacterEntity()->SetDrawFlag(true);
//				//panel->SetDrawFlag(true);
...
//	
//	
//	UIMeshComponent * textBoxMeshComponent =	GetUiMeshComponent();
//
//
//
//	textBoxMeshComponent->SetTexture("TextBox.png");
//	textBoxMeshComponent->SetEffect("DefaultUi.effect");
...
//	mTextCursorEntity->GetUiMeshComponent()->SetIndependentScaleFlag(true);
//	mTextCursorEntity->GetUiComponent()->SetSelectAvailableFlag(false);
//	//mTextCursorEntity->SetSize(1.0f, GetLineHeight());
//	mTextCursorEntity->GetUiMeshComponent()->SetWidthHeightLocal(1.0f, GetLineHeight());
//	AddChildObject(mTextCursorEntity);
//	SetCursorPosition(0);
//
//
