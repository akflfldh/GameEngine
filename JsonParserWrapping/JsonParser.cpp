...
template <typename T> inline bool Parser::GetValue(const JsonPath &key, T &oValue) const
{
...
template <typename T> bool Parser::GetValueFromReadPointer(T &oValue) const
{
...
size_t Parser::GetElementNum() const
{
    return pImpl->GetElementNum();
}
