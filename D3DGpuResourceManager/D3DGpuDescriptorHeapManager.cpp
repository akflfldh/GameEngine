...
D3DGRM::D3DGpuDescriptorHeapManager::D3DGpuDescriptorHeapManager(Microsoft::WRL::ComPtr<ID3D12Device> device,
                                                                 D3D12_DESCRIPTOR_HEAP_TYPE type, size_t descriptorNum,
...
D3DGRM::D3DDescriptorHandle D3DGRM::D3DGpuDescriptorHeapManager::Allocate()
{
    ...
    handle.mCpuDescriptorHandle.ptr =
        mDescriptorHeap->GetCPUDescriptorHandleForHeapStart().ptr + offset * mDescriptorIncrementSize;

    if (mHeapFlag == D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE)
    {
        // visible일경우에만 gpuDescriptorHandle이 유효하다.
        handle.mGpuDescriptorHandle.ptr =
            mDescriptorHeap->GetGPUDescriptorHandleForHeapStart().ptr + offset * mDescriptorIncrementSize;
    }
    else
