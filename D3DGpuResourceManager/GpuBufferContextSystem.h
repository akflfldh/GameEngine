...
struct GPURESOURCE_MANAGER_API AllocateRange
{

    AllocateRange();
    // 맨처음 nextOffset을 리턴한다.
    // 일단 모자르면 assert(0)
    uint32_t UseRange(uint32_t usageCount) const;

    // nextOffset = 0으로 초기화 ,매프레임 GpuBufferContextSystem이 호출할것
    void NextFrame();
    void Reset();
    void SetTotalSize(uint32_t end);

    // 범위는 [0,1,2]
    uint32_t GetCurrentFrameIndex(uint32_t relativeOffset) const;
  ...
  private:
    ...
    uint32_t mEnd = 0;
    mutable uint32_t mNextOffset = 0;
    int mFrameIndex = 0;
};

struct GPURESOURCE_MANAGER_API GpuBufferContext
{
    uint32_t mID;
    ...
    // 버퍼의 nextIndex를 매프레임 0으로초기화에대한 정책
    // 0 : clear, 1:  keep
    int mBufferIndexClearPolicy = 0;
};

struct GPURESOURCE_MANAGER_API GpuConstantBufferContext : public GpuBufferContext
{
    GRM::GRMPtr mGpuBuffer;
    AllocateRange mAllocateRange;
    // AllocateRange  :  사용한 범위, 가용범위 (RenderItemBuilder에서 RenderItem마다 적절한 buffer의 offset start값과
    // num값을 지정해야하니,  또 아마 매프레임 GpuBufferContextSystem에 의해서 reset될거임.
};

struct GPURESOURCE_MANAGER_API GpuStructuredBufferContext : public GpuBufferContext
{
    std::vector<GRM::GRMPtr> mGpuBuffersPerFrame;
    bool mIsFrameBuffers; // false -> GpuBuffers size : 1  , true : 3(frameCount)
...
class GPURESOURCE_MANAGER_API GpuBufferContextSystem
{
  public:
    ...
    virtual void LoadShaderBufferFile(const std::filesystem::path &filePath) = 0;

    // 이메서드는 에디터와, 유저 DLL의 전역함수가 호출한다.
    // 이미 등록되어있는 버퍼에대해  버퍼를 채울 데이터메모리를 반환하는 콜백 등록을 수행한다.
    virtual bool RegisterGpuBufferCallback(uint32_t id) = 0;
