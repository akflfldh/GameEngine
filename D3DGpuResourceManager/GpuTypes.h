...
namespace GRM
{
	...
	enum class ETextureFormat : unsigned int
	{
	...
	enum class ETextureUsage : unsigned int
	{
		eNone = 0,
	...
	struct GPURESOURCE_MANAGER_API ScratchImage
	{
		size_t      mimagesNum;
		size_t      mSize;	//전체픽셀사이즈
		...
		ScratchImage(const ScratchImage& rhs);
		ScratchImage& operator=(const ScratchImage& rhs);
	...
	GPURESOURCE_MANAGER_API bool IsSRGBType(ETextureFormat format)  ;
