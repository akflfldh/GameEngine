...
void D3DRender::D3DRenderChannelSystem::BeginFrame(
	const Render::FrameContext& frameContext)
{
	...
	D3DGRM::D3DGpuTexture* backBuffer =static_cast<D3DGRM::D3DGpuTexture*>(windowRenderData->GetBackBuffer(mChannelFrameContext.mCurrentWindowBackBufferIndex).getResource());
	...
	depthStencilBuffer->GetDescriptorHandle(D3DGRM::ED3DResourceDescriptorType::eDSV, depthStencilBufferDescriptorHandle);


	mCommandList->ClearRenderTargetView(backBufferDescriptorHandle.mCpuDescriptorHandle, mChannelFrameContext.mBackBufferClearColor, 1, nullptr);
	mCommandList->ClearDepthStencilView(depthStencilBufferDescriptorHandle.mCpuDescriptorHandle, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL,
		mChannelFrameContext.mDepthClearValue, mChannelFrameContext.mStencilClearValue, 1, nullptr);
...
void D3DRender::D3DRenderChannelSystem::SubmitRenderItem(const std::vector<Render::InternalRenderItem>& renderItemVector)
{
