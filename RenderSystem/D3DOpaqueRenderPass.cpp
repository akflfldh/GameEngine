...
D3DRender::D3DOpaqueRenderPass::D3DOpaqueRenderPass(Microsoft::WRL::ComPtr<ID3D12Device> device, D3DGRM::D3DGpuResourceManager* resourceManager, std::shared_ptr<D3DRender::D3DMaterialManager> materialManager,
	D3DGRM::D3DGpuTexture* windowBackBuffer[2], D3DGRM::D3DGpuTexture* windowDepthStencilBuffer)
	:ID3DRenderPass(materialManager),mDevice(device),mResourceManager(resourceManager),mWindowDepthStencilBuffer(windowDepthStencilBuffer)
{
...
void D3DRender::D3DOpaqueRenderPass::BeginPass(ID3D12GraphicsCommandList* commandList, const D3DRender::D3DRenderChannelFrameContext& channelFrameContext)
{
	//기본설정
	...
	//commandList->RSSetScissorRects(1, reinterpret_cast<const D3D12_RECT*>(&channelFrameContext.mFrameContext.mScissorRect));
	...
	mWindowBackBuffer[channelFrameContext.mCurrentWindowBackBufferIndex]->GetDescriptorHandle(D3DGRM::ED3DResourceDescriptorType::eRTV, renderTargetHandle);
	mWindowDepthStencilBuffer->GetDescriptorHandle(D3DGRM::ED3DResourceDescriptorType::eDSV, depthStencilHandle);

	commandList->OMSetRenderTargets(1, &renderTargetHandle.mCpuDescriptorHandle, true, &depthStencilHandle.mCpuDescriptorHandle);
	//깊이 버퍼도 설정
...
void D3DRender::D3DOpaqueRenderPass::Draw(ID3D12GraphicsCommandList* commandList, const std::vector<Render::InternalRenderItem>& renderItems)
{
	//각 렌더아이템이 참조하는 머터리얼의 pso rootsignature , shader resource binding 작업과 draw수행


	bool firstRenderItemFlag = true;

	
	
	Render::InternalRenderItem dummyRenderItem;
	memset(&dummyRenderItem, 0, sizeof(dummyRenderItem));

	const Render::InternalRenderItem* beforeRenderItem = &dummyRenderItem;

	D3DMaterialItem beforeMaterialItem;
	memset(&beforeMaterialItem, 0, sizeof(beforeMaterialItem));
	...
	for(size_t renderItemIndex= 0; renderItemIndex < renderItems.size(); ++renderItemIndex)
	{
		...
		D3DMaterialItem currMaterialItem;
		mMaterialManager->GetMaterialItem(currRenderItem.mMaterialID, currMaterialItem);

		const D3DMainRenderPassInfo& currMatMainPass = currMaterialItem.mMainPass;
		const D3DMainRenderPassInfo& beforeMatMainPass = beforeMaterialItem.mMainPass;
		...
		//머터리얼 요소비교
		if (beforeRenderItem->mMaterialID != currRenderItem.mMaterialID)
		{
			//이전 PSO와 비교
		...
		for (const auto& resourceElement : currRenderItem.mBindingGpuResourceVector)
		{
			...
			std::unordered_map<std::string, int>::const_iterator rootParameterIt =	 currMatMainPass.mShaderResourceRootParameterBindingInfo.find(resourceElement.mName);
		...
		if (beforeRenderItem->mMeshItem != currRenderItem.mMeshItem)
		{
			D3DGRM::D3DGpuVertexBuffer* vertexBuffer = static_cast<D3DGRM::D3DGpuVertexBuffer*>(currRenderItem.mMeshItem.mVertexBuffer);
			D3DGRM::D3DGpuIndexBuffer* indeBuffer = static_cast<D3DGRM::D3DGpuIndexBuffer*>(currRenderItem.mMeshItem.mIndexBuffer);
		...
		commandList->DrawIndexedInstanced(currRenderItem.mMeshItem.mIndexNum,
			currRenderItem.mInstance.mInstanceCount, currRenderItem.mMeshItem.mIndexOffset, currRenderItem.mMeshItem.mVertexOffset,
