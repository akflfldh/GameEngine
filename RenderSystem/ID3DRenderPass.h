...
namespace D3DRender
{
	...
	public:
		...
		//여러가지 설정등
		virtual void BeginPass(ID3D12GraphicsCommandList* commandList,const D3DRenderChannelFrameContext& frameContext) = 0;


		//renderItem들을 받아서 실제 드로우
		virtual void Draw(ID3D12GraphicsCommandList* commandList, const std::vector<Render::InternalRenderItem>& renderItems) = 0;
