...
bool QuadPF::WindowsPhysicalFileSystem::CreatePhysicalFile(const std::string& fileName, const std::string& path)
{
	...
	WindowHandleRef handle =  CreateFile(fileNameFullPath.c_str(), GENERIC_READ | GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
bool QuadPF::WindowsPhysicalFileSystem::CreatePhysicalFolder(const std::string& folderName, const std::string& path)
{
	...
	if (CreateDirectory(folderFullPath.c_str(), nullptr) == 0)
	{
...
bool QuadPF::WindowsPhysicalFileSystem::GetFileListByExtension(const std::string& folderPath, const std::string& extension,
...
void QuadPF::WindowsPhysicalFileSystem::LogError() const
{
	...
	TCHAR* outputErrorBuffer = nullptr;

	//log
	DWORD errorStrNum = FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, nullptr, GetLastError(), 0, (LPTSTR)(&outputErrorBuffer), 0, nullptr);
...
HANDLE QuadPF::WindowsPhysicalFileSystem::OpenFile(const std::string& path , DWORD creationFlag)
{
	...
	HANDLE handle =	CreateFile(filePathW.c_str(), GENERIC_READ | GENERIC_WRITE, 0, nullptr, creationFlag, FILE_ATTRIBUTE_NORMAL, NULL);
