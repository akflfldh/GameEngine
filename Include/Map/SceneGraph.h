...
namespace Quad
{
	...
	namespace Scene
	{
		...
		public:
			...
			bool Add(Object* ob);
			void RemoveKilledObjectAll();//kill상태인 오브젝트들의 노드를 모두제거한다.
		...
		private:
			...
			Scene::Node* findNode(Object* object) const;
			Scene::Node* findNode(unsigned long long id);

			Scene::Node* FindAncestorNode(Scene::Node* descendantNode, Object* ancestorObject);//없다면 nullptr리턴
			bool IsAncestorNode(Scene::Node* descendantNode, Scene::Node* ancestorObject);//조상노드인가의 여부


			void RemoveNodeAll(Scene::Node* node);//해당노드 + 모든 자식노드를 제거한다.
