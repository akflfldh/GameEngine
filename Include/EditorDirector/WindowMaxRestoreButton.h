...
namespace Quad
{
	class WindowMaxRestoreButton :public WindowButton
	{
	public:
		WindowMaxRestoreButton(const std::string& name="");
		virtual ~WindowMaxRestoreButton() = default;

		virtual void Initialize() override;
		static WindowMaxRestoreButton* Create(Map* map, int mapLayerIndex = 0);
	...
	//	virtual void SetWidthHeight(float width, float height)override;
		...
		CLASSSIZE(WindowMaxRestoreButton)
		CLASSNAME(WindowMaxRestoreButton)
	protected:
	...
	REGISTEREDITORCLASS(WindowMaxRestoreButton)
