...
namespace QuadLF
{
	...
	public:
		...
		//physcialRootPath  최상위 루프폴더가 위치한물리주소(논리적파일시스템이 물리적파일시스템을 사용하지않는다면 빈문자열을 전달하면된다)
		//ex) C:/aaa/bbb/ccc	맨 뒤 '/' 룰 붙이지 않는다
		...
		LogicalFile* MakeFile(const LogicalFileAssetInfo& logicalAssetFileInfo,
			const std::string & fileName,const std::string & path ,bool physicalFileFlag = false);
		LogicalFile* MakeFile(const LogicalFileAssetInfo & logicalAssetFileInfo,
			const std::string & fileName, LogicalFolder * parentFolder,bool physicalFileFlag = false);
		QuadLF::LogicalFolder* CreateFolder(const std::string & folderName, LogicalFolder * parentFolder, bool physicalFolderFlag = false);
		...
		std::vector<LogicalFile*> GetFile(const std::string& fileName, uint32_t assetTypeFilter, LogicalFolder* parentFolder = nullptr);
		...
		bool SaveLogicalDirectoryStructureAsBinaryWriter(QuadRW::BinaryWriter & binaryWriter, const std::string & path , const std::string & fileName);
	...
	private:
		...
		LogicalFolder * CreateLogicalFolder(LogicalFolder * parentFolder,  const std::string & folderName, bool physicalFolderBindingFlag);
