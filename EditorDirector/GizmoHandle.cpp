...
//{
//
//	MeshComponent * meshComponent =AddComponent<MeshComponent>();
//	//meshCOmponent는 있는데 mesh는없고 충돌체(볼륨)만있는
...
//	//->RemoveComponent(EComponentType::eMeshComponent);
//
//	//충돌체는 가지고있어야겠다.
...
//	MeshComponent* coneMeshComponent = mCone->GetComponent<MeshComponent>();
...
//
//	MeshComponent* coneMeshComponent = mCone->GetComponent<MeshComponent>();
//	coneMeshComponent->SetMesh(MeshManager::GetMesh("Cone"));
//	mCone->SetEngineObjectFlag(true);
//
//
//	mCone->GetTransform().SetPositionLocal({ 0,5,0 });
//	MeshComponent* coneMeshComponent = coneMeshComponent;
//	coneMeshComponent->GetSubMesh()[0].mModelMaterial.SetEffectName("Gizmo.effect");
//
//
//
...
//
//
//	BoxCollider* collider= static_cast<BoxCollider*>(GetComponent<MeshComponent>()->GetCollider());
//	collider->SetMinMaxVertex(minMaxPoint.first, minMaxPoint.second);
//	//colliderComponent->SetCollider(ColliderGenerator::CreateBoxCollider(minMaxPoint.first, minMaxPoint.second));
//
//
//
